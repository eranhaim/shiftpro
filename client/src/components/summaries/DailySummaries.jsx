...
import { RefreshCw, AlertTriangle, FileText, Plus, Pencil, Download, Filter, X, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
export default function DailySummaries() {
  ...
  return (
    <div className="space-y-6 pb-24 p-4 lg:p-6" dir="rtl">
                ...
                {summaries.map((s) => {
                  ...
                  return (
                    <tr key={s._id} className="hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="py-2.5 px-4 whitespace-nowrap">
                        <button onClick={() => setViewingSummary(s)} className="text-gray-400 hover:text-blue-400 transition-colors" title="צפה בדו״ח">
      ...
      {/* View Summary Modal */}
      {viewingSummary && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-2 sm:p-4" onClick={() => setViewingSummary(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 sticky top-0 bg-gray-900 z-10">
              <div>
                <h2 className="text-lg font-bold text-white">{viewingSummary.chatterId?.name || 'לא ידוע'}</h2>
                <p className="text-sm text-gray-400">{formatDate(viewingSummary.date)} · {viewingSummary.shiftType}</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { setViewingSummary(null); openEdit(viewingSummary); }}
                ...
                <button onClick={() => setViewingSummary(null)} className="text-gray-400 hover:text-white p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="p-6 space-y-5">
              {/* הכנסות */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">הכנסות</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">טלגרם</p>
                    <p className="text-white font-bold">${(viewingSummary.incomeTelegramUSD || 0).toFixed(2)}</p>
                    <p className="text-xs text-gray-600">€{viewingSummary.incomeTelegram || 0}</p>
                  </div>
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">אונליפאנס</p>
                    <p className="text-white font-bold">${(viewingSummary.incomeOnlyfansUSD ?? viewingSummary.incomeOnlyfans ?? 0).toFixed(2)}</p>
                  </div>
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">העברות</p>
                    <p className="text-white font-bold">${(viewingSummary.incomeTransfersUSD || 0).toFixed(2)}</p>
                    <p className="text-xs text-gray-600">₪{viewingSummary.incomeTransfers || 0}</p>
                  </div>
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">אחר</p>
                    <p className="text-white font-bold">${(viewingSummary.incomeOtherUSD || 0).toFixed(2)}</p>
                    <p className="text-xs text-gray-600">₪{viewingSummary.incomeOther || 0}</p>
                  </div>
                </div>
                <div className="mt-2 bg-gray-800/50 rounded-lg px-4 py-2 flex justify-between">
                  ...
                  <span className="text-green-400 font-bold">${(viewingSummary.incomeTotalUSD || viewingSummary.incomeTotal || 0).toFixed(2)}</span>
                </div>
              </div>

              {/* זמינות */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">זמינות</h3>
                <div className="bg-gray-800 rounded-lg px-4 py-3 flex items-center gap-3">
                  {viewingSummary.availabilityStatus === 'full' && <CheckCircle className="w-4 h-4 text-green-400" />}
                  {viewingSummary.availabilityStatus === 'partial' && <AlertCircle className="w-4 h-4 text-yellow-400" />}
                  {viewingSummary.availabilityStatus === 'absent' && <XCircle className="w-4 h-4 text-red-400" />}
                  <span className="text-white text-sm">
                    {viewingSummary.availabilityStatus === 'full' ? 'מלאה' : viewingSummary.availabilityStatus === 'partial' ? 'חלקית' : 'נעדר'}
                  </span>
                  {viewingSummary.availabilityGapsDetail && (
                    <span className="text-gray-400 text-sm">— {viewingSummary.availabilityGapsDetail}</span>
                  )}
                </div>
              </div>
              ...
              {/* דגלים */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">סטטוסים</h3>
                <div className="space-y-2">
                  <div className={`flex items-center gap-3 rounded-lg px-4 py-3 ${viewingSummary.hasDebts ? 'bg-red-900/20 border border-red-800/40' : 'bg-gray-800'}`}>
                    {viewingSummary.hasDebts ? <XCircle className="w-4 h-4 text-red-400 shrink-0" /> : <CheckCircle className="w-4 h-4 text-green-400 shrink-0" />}
                    <div>
                      <p className="text-sm text-white">חובות</p>
                      {viewingSummary.hasDebts && viewingSummary.debtsDetail && <p className="text-xs text-gray-400 mt-0.5">{viewingSummary.debtsDetail}</p>}
                    </div>
                  </div>
                  <div className={`flex items-center gap-3 rounded-lg px-4 py-3 ${viewingSummary.hasPendingSales ? 'bg-yellow-900/20 border border-yellow-800/40' : 'bg-gray-800'}`}>
                    {viewingSummary.hasPendingSales ? <AlertCircle className="w-4 h-4 text-yellow-400 shrink-0" /> : <CheckCircle className="w-4 h-4 text-green-400 shrink-0" />}
                    <div>
                      <p className="text-sm text-white">מכירות תלויות</p>
                      {viewingSummary.hasPendingSales && viewingSummary.pendingSalesDetail && <p className="text-xs text-gray-400 mt-0.5">{viewingSummary.pendingSalesDetail}</p>}
                    </div>
                  </div>
                  <div className={`flex items-center gap-3 rounded-lg px-4 py-3 ${viewingSummary.hasUnusualEvents ? 'bg-orange-900/20 border border-orange-800/40' : 'bg-gray-800'}`}>
                    {viewingSummary.hasUnusualEvents ? <AlertTriangle className="w-4 h-4 text-orange-400 shrink-0" /> : <CheckCircle className="w-4 h-4 text-green-400 shrink-0" />}
                    <div>
                      <p className="text-sm text-white">אירועים חריגים</p>
                      {viewingSummary.hasUnusualEvents && viewingSummary.unusualEventsDetail && <p className="text-xs text-gray-400 mt-0.5">{viewingSummary.unusualEventsDetail}</p>}
                    </div>
                  </div>
                  <div className={`flex items-center gap-3 rounded-lg px-4 py-3 ${viewingSummary.allDepositsVerified ? 'bg-green-900/20 border border-green-800/40' : 'bg-gray-800'}`}>
                    {viewingSummary.allDepositsVerified ? <CheckCircle className="w-4 h-4 text-green-400 shrink-0" /> : <XCircle className="w-4 h-4 text-red-400 shrink-0" />}
              ...
              {/* הערות */}
              {(viewingSummary.improvementSuggestions || viewingSummary.contentRequest || viewingSummary.selfImprovementPoint || viewingSummary.selfPreservationPoint) && (
                <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">הערות</h3>
                  <div className="space-y-2">
                    {viewingSummary.improvementSuggestions && (
                      <div className="bg-gray-800 rounded-lg px-4 py-3">
                        <p className="text-xs text-gray-500 mb-1">הצעות לשיפור</p>
                        <p className="text-sm text-gray-300">{viewingSummary.improvementSuggestions}</p>
                      </div>
                    )}
                    {viewingSummary.contentRequest && (
                      <div className="bg-gray-800 rounded-lg px-4 py-3">
                        <p className="text-xs text-gray-500 mb-1">בקשת תוכן</p>
                        <p className="text-sm text-gray-300">{viewingSummary.contentRequest}</p>
                      </div>
                    )}
                    {viewingSummary.selfImprovementPoint && (
                      <div className="bg-gray-800 rounded-lg px-4 py-3">
                        <p className="text-xs text-gray-500 mb-1">נקודת שיפור עצמי</p>
                        <p className="text-sm text-gray-300">{viewingSummary.selfImprovementPoint}</p>
                      </div>
                    )}
                    {viewingSummary.selfPreservationPoint && (
                      <div className="bg-gray-800 rounded-lg px-4 py-3">
                        <p className="text-xs text-gray-500 mb-1">נקודת שימור עצמי</p>
                        <p className="text-sm text-gray-300">{viewingSummary.selfPreservationPoint}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="number" min="0" value={form.incomeTelegram} onChange={(e) => setForm({ ...form, incomeTelegram: e.target.value })} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg pr-7 pl-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="number" min="0" value={form.incomeOnlyfans} onChange={(e) => setForm({ ...form, incomeOnlyfans: e.target.value })} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg pr-7 pl-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="number" min="0" value={form.incomeTransfers} onChange={(e) => setForm({ ...form, incomeTransfers: e.target.value })} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg pr-7 pl-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-400 text-sm font-medium">₪</span>
                      <input type="number" min="0" value={form.incomeOther} onChange={(e) => setForm({ ...form, incomeOther: e.target.value })} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg pr-7 pl-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
                    </div>
                    <p className="text-xs text-gray-600 mt-0.5">כולל מע&quot;מ 18%</p>
                  </div>
                </div>
