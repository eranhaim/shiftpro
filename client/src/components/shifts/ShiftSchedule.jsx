...
export default function ShiftSchedule() {
  ...
  const selectedDayShifts = selectedDay
    ? shifts.filter((s) => (s.date?.split('T')[0] || s.date) === toISODate(selectedDay))
    : [];
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-center gap-4">
        <button onClick={() => navigateWeek(-1)} className="text-gray-400 hover:text-white p-1"><ChevronRight className="w-5 h-5" /></button>
        <button onClick={goToCurrentWeek} className="text-blue-400 hover:text-blue-300 font-medium">השבוע</button>
        <span className="text-gray-300 font-medium">{formatDateRange(weekStart)}</span>
